...
<section id="rar-dashboard" class="rar-view active">
    <div class="rar-stats">
        <div class="rar-stat"><span>Today's Orders</span><strong id="stat-orders">—</strong></div>
        <div class="rar-stat"><span>Today's Sales</span><strong id="stat-sales">—</strong></div>
        <div class="rar-stat"><span>Low Stock</span><strong id="stat-low">—</strong></div>
        <div class="rar-stat"><span>Out of Stock</span><strong id="stat-out">—</strong></div>
    </div>
    <div class="rar-actions-home">
        <?php if ( RAR_WSO_Plugin::can( 'rar_wso_manage_stock' ) ) : ?>
            <button class="rar-big-btn" data-view="stock"><span>📦</span>Stock Manager</button>
        <?php endif; ?>
        <?php if ( RAR_WSO_Plugin::can( 'rar_wso_create_orders' ) ) : ?>
            <button class="rar-big-btn" data-view="order"><span>🧾</span>Create Order</button>
        <?php endif; ?>
    </div>
...
<section id="rar-order" class="rar-view">
    ...
    <form id="rar-order-form" novalidate>
        <div class="rar-card rar-customer-grid">
            <label><span>Customer name *</span><input name="name" autocomplete="name" required></label>
            <label><span>Phone *</span><input name="phone" inputmode="tel" autocomplete="tel" required></label>
            <label><span>Email (optional)</span><input name="email" type="email" autocomplete="email"></label>
            <label class="wide"><span>Address *</span><input name="address" autocomplete="street-address" required></label>
            <label><span>Town / City *</span><input name="city" autocomplete="address-level2" required></label>
            <label><span>District *</span><input name="district" list="rar-districts" autocomplete="off" required></label>
            <datalist id="rar-districts">
                <?php foreach ( $districts as $code => $label ) : ?>
        ...
        <div class="rar-card">
            <label><span>Search product</span><input id="rar-order-search" type="search" placeholder="Type item name or SKU…" autocomplete="off"></label>
        ...
        <div class="rar-card">
            ...
            <label><span>Order note (optional)</span><textarea name="note" rows="3"></textarea></label>
        </div>

        <button class="rar-primary rar-save-order" type="submit">Save Order</button>
    </form>
