...
function productRow(product){
    ...
    return '<div class="rar-product-row" data-id="'+product.id+'">'+
        ...
        '<div class="rar-stock-control">'+
            ...
            (C.allowDelete?'<button class="rar-delete" data-delete="'+product.id+'">Trash</button>':'')+
        '</div>'+
    '</div>';
}
...
$('#rar-stock-list')?.addEventListener('click',async event=>{
    ...
    if(del){
        ...
        try{
            const data=await api('delete_product',{product_id:del.dataset.delete});
            del.closest('.rar-product-row')?.remove();
            toast(data.message);
        }catch(error){
            toast(error.message);
        }
...
$('#rar-shipping')?.addEventListener('input',calculateTotals);
...
$('#rar-order-form')?.addEventListener('submit',async event=>{
    ...
    try{
        ...
        result.innerHTML='<strong>Order #'+esc(data.order_num)+' created</strong><br>Total: '+esc(data.total)+'<br><a href="'+esc(data.admin_url)+'" target="_blank" rel="noopener">Open in WooCommerce</a>';
        form.reset();
...
const modal=$('#rar-add-product-modal');

$('#rar-add-product-open')?.addEventListener('click',()=>{modal.hidden=false;});
$('#rar-add-product-close')?.addEventListener('click',()=>{modal.hidden=true;});
modal?.addEventListener('click',event=>{
    if(event.target===modal)modal.hidden=true;
});

$('#rar-add-product-form')?.addEventListener('submit',async event=>{
    event.preventDefault();
    const form=event.currentTarget;
    if(!form.reportValidity())return;
